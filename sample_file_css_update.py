...
def load_existing_favorites():
    ...
    if os.path.exists(FAVORITES_DIR):
        for f in os.listdir(FAVORITES_DIR):
            if f.endswith(".png"):
                path = os.path.abspath(os.path.join(FAVORITES_DIR, f))
                favorites.append((path, f))
    return favorites
...
def append_to_favorites(target_img, custom_name, _):
    if not target_img:
        return load_existing_favorites(), gr.update(), '<div style="color: #f87171;">Nothing to save.</div>'
    ...
    return updated_favs, gr.update(value=updated_favs), '<div style="color: #4ade80;">Saved to persistent gallery!</div>'
...
# def set_scroll_notice():
#     return '<div style="text-align: center; font-weight: bold; color: #38bdf8; background: rgba(56, 189, 248, 0.15); border: 1px solid #38bdf8; padding: 8px; border-radius: 8px; margin-top: 15px; margin-bottom: 25px; box-shadow: 0 0 10px rgba(56, 189, 248, 0.2);">⬇ Please scroll down to view the generated images below!</div>'

def clear_scroll_notice():
    return ""
...
def create_generator_ui():
    ...
    with gr.Tabs():
        with gr.TabItem("Studio Workspace"):
            ...
            # --- ROW 1: GALLERY & EDITING PANEL (TOP SECTION) ---
            with gr.Row():
            ...
            # --- ROW 2: GENERATION INPUT PANEL (BOTTOM SECTION) ---
        ...
        with gr.TabItem("Saved Gallery"):
            with gr.Group(elem_classes=["fav-matrix-container"]):
                ...
                saved_gallery = gr.Gallery(
                    ...
                    label="Archived Asset Scrapbook",
                    columns=10,
                    type="filepath",
                    height="auto"
                )
...
with gr.Blocks(css=GENERATOR_CSS) as demo:
    ...
    ui["generate_btn"].click(
        ...
        fn=clear_scroll_notice,
        inputs=None,
        outputs=[ui["scroll_notice"]]
    )
